...
from ...domain.gateway.zipcode_distance_calculator_api import (
    ZipcodeDistanceCalculatorApi,
)
...
class PlaceOrder:
    ...
    def execute(self, place_order_input: PlaceOrderInput) -> PlaceOrderOutput:
        """Place order according to the DTO received, saving the order created
        to the database.

        Args:
            place_order_input (PlaceOrderInput): DTO of the order

        Returns:
            PlaceOrderOutput: Created order output
        """
        order_data = place_order_input.dict()
        order_service = OrderCreator(
            self.__repository_factory, self.__zipcode_calculator
        )
