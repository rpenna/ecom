...
def test_given_an_order_when_its_placed_then_it_should_return_expected_total_price(place_order, place_order_input):
    output = place_order.execute(place_order_input)
    assert output.total_price == Monetary(349.77)

def test_given_an_order_when_its_placed_then_it_should_return_expected_shipping_fee(place_order, place_order_input):
    output = place_order.execute(place_order_input)
    assert output.shipping_fee == Monetary(400)

def test_given_an_order_made_in_default_month_when_its_placed_then_it_should_return_expected_tax(place_order, place_order_input):
    output = place_order.execute(place_order_input)
    assert output.tax == Monetary(23.87)

def test_given_an_order_made_in_default_month_when_its_placed_then_it_should_return_expected_tax(place_order, place_order_input_november):
    output = place_order.execute(place_order_input_november)
    assert output.tax == Monetary(2.3794)

def test_given_an_order_with_more_quantity_than_available_in_stock_when_its_placed_then_it_shoul_raise_out_of_stock_exception(place_order):
    products = [
        {
            'id': '2',
            'quantity': 300
