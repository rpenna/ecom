...
def order_three_products():
    ...
    order.add_to_cart("1", 1990, 1)
    order.add_to_cart("2", 280, 20)
    order.add_to_cart("3", 22799, 1)
    return order


def test_should_accept_order_with_three_products(order_three_products):
    assert order_three_products.get_total_price() == 30389
...
def test_should_apply_discount(order_three_products):
    ...
    assert order_three_products.get_total_price() == 27351
